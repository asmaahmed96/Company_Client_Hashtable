...
//PURPOSE: search list to see if node has key, if found return element, if not return blank element
//PARAMETER: passes key to be found
el_t slist::search2(int key)
{
  Node *P; //temp node pointer to traverse in list
  P = Front;
  int counter = 0;
  while(P!=NULL)
    {
      if(P->Elem.getrank() == key)
	return P->Elem;
      else
	counter++;
	P=P->Next;
  ...
  el_t blank;
  return blank; //return blank if element not found
}
